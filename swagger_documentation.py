...
def login_user():
    ...
    responses:
      400: 
        description: Key variables to be entered are supposed to be user_email and user_password. The email has to be valid.
      401:
...
def get_bucketlist():
    ...
    parameters:
      - name: Authorization
        description: Token that should begin with the word Bearer and a space after to separate the two statements 'Bearer '
        in: header
        type: string
        required: true
      - name: q
...
def manipulate_bucketlist(id):
  ...
  parameters:
    ...
    - name: Authorization
      description: Token that should begin with the word Bearer and a space after to separate the two statements 'Bearer token'
      in: header
      type: string
      required: true
  responses:
...
def update_bucketlist(id):
  ...
  The route updates a bucket using the id of the bucket to determine the exact bucket and a name to replace the original name of the bucket
  ---
  parameters:
    - name: id
      in: path
...
def post_activities(bucket_id):
  ...
  parameters:
    - name: activity_name
      in: formData
      type: string
      description: An activity within the bucketlist, that does not exist in the specific bucketlist
      required: true
    - name: bucket_id
      in: path
      type: integer
  ...
  responses:
    ...
    409:
      description: Activity not created because it already exists in this users bucket list as it was created before
  """
  pass

@app.route('/bucketlists/<int:bucket_id>/items/', methods=['GET'])
def get_activities(bucket_id):
  ...
  responses:
    ...
    400:
      description: Bad reqeust possibly because wrong id supplied for the bucket list, no data given for search parameter
    200:
...
def manipulate_activity(bucket_id, activity_id):
  ...
  responses:
    ...
    400:
      description: Bad request, bucket id does not exist for this user, activity id does not exist for this user
    200:
...
def update_activity(bucket_id, activity_id):
  ...
  responses:
    401: 
      description: User unauthorized
    400:
      description: Bad request the bucket does not exist or the activity does not exist from the respective ids given by the user
    409:
      description: Activity name had been entered before posing conflict with new activity name inserted
    200:
